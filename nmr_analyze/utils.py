...
def raw_nmr_to_dataframe(file_path, reagents):
    ...
    df = pd.DataFrame(
        columns=["experimental_spectrum", "theoretical_spectrum", "reactivity_label"]
    )
...
def read_data(dataframe):
    ...
    for line in range(len(dataframe)):
        # Read the spectra
        experimental_spectrum = np.array(
            eval(dataframe.loc[line].experimental_spectrum)
        )
        ...
        if len(experimental_spectrum) == 4878:
            experimental_spectrum = signal.resample_poly(
                experimental_spectrum, 1000, 18040
            )
            theoretical_spectrum = signal.resample_poly(
                theoretical_spectrum, 1000, 18040
            )
        else:
        ...
        concatented = np.concatenate(
            (theoretical_spectrum, experimental_spectrum), axis=0
        )
...
def training_validation_test_split(
    data_x, data_y, training_size, validation_size, seed=125
):
    ...
    training_set_x, training_set_y = (
        data_x[training_set_idxs],
        data_y[training_set_idxs],
    )
    validation_set_x, validation_set_y = (
        data_x[validation_set_idxs],
        data_y[validation_set_idxs],
    )
...
def expanddataset(datax, datay):
    ...
    newdatax = np.concatenate(
        (datax, exp_data, shifted_data, shifted_data1, shifted_data2), axis=0
    )
