...
def Conv2D(inputs, params, use_pooling=False, activation_fn=tf.nn.relu,
    ...
    with tf.name_scope(name):
        ...
        # Do the convolution
        layer1 = tf.nn.conv2d(inputs, filter=weights, strides=strides,
        ...
        if use_pooling:
            ...
            layer1 = tf.nn.avg_pool(value=layer1, ksize=pool_kernel,
...
class NMR_nn():
    ...
    def build_model(self, x, phase):
        ...
        inputs_noisy = tf.cond(tf.equal(phase, tf.constant(True, tf.bool)),
                               lambda: noisy(inputs, self.noise_level),
                               lambda: no_noise(inputs))

        params1a = {'weights': [2, 2, 1, 16], 'strides': [1, 1, 1, 1],
                    'pool_kernel': [1, 1, 2, 1], 'pool_strides': [1, 1, 2, 1]}
        layer1a = Conv2D(inputs_noisy, params1a, use_pooling=True, name='Layer1a')

        params1b = {'weights': [2, 10, 1, 16], 'strides': [1, 1, 1, 1],
                    'pool_kernel': [1, 1, 2, 1], 'pool_strides': [1, 1, 2, 1]}
        layer1b = Conv2D(inputs_noisy, params1b, use_pooling=True, name='Layer1b')

        params1c = {'weights': [2, 20, 1, 16], 'strides': [1, 1, 1, 1],
                    'pool_kernel': [1, 1, 2, 1], 'pool_strides': [1, 1, 2, 1]}
    ...
    def train(self, trsetx, trsety, valsetx, valsety):
        ...
        cross_entropy = -(y_true * tf.log(y_pred + 1e-12) + (1 - y_true) * tf.log(1 - y_pred + 1e-12))
        ...
        for e in range(self.num_epochs):
            ...
            for i in range(iter_per_epoch):
                ...
                _, iloss = sess.run([optimize, loss], feed_dict={x: batchx, y_true: batchy,
                                                                 phase: True})
                #                 print ('loss:\t ', iloss)
                e_losses.append(iloss)
            print('Epoch {} training loss {}'.format(e, np.mean(e_losses)))

            val_loss = sess.run(loss, feed_dict={x: valsetx,
                                                 y_true: valsety, phase: False})
    ...
    def test(self, testx, testy):
        ...
        top2_accuracy = tf.reduce_mean(tf.cast(tf.nn.in_top_k(predictions=y_pred,
                                                              targets=tf.argmax(y_true, axis=-1), k=2),
                                               tf.float32))
        accuracy = tf.reduce_mean(tf.cast(tf.nn.in_top_k(predictions=y_pred,
                                                         targets=tf.argmax(y_true, axis=-1), k=1),
                                          tf.float32))
