...
reactions = [i for i in glob.glob(os.path.join(MAIN_FOLDER,'*_HPLC')) if 'BLANK' not in i]
reactions_blanks = glob.glob(os.path.join(MAIN_FOLDER,'*BLANK_HPLC'))
reagents = [i for i in glob.glob(os.path.join(REAGENTS_FOLDER,'*_HPLC')) if 'BLANK' not in i]
...
def get_reagents(file):
    reagents = [x for x in path.basename(file).split("_")[1].split("-") if x not in ['1','5','16']] # 
    return reagents
...
def makelib(spectra, regions): #makes a list of regions [[[points of region],start,end],[points of region],start,end]], [...]] for all reagents
...
def synthesize(reg, shifts, length):
    ...
    for i,r in enumerate(reg):
        ...
        if merged.shape[0] > length:
            merged = np.array([3000 for i in range(length)]) #setting it crazy high so optimizer goes away
...
def get_error(x, *params): #x=list of parameters
    ...
    spec, lib = params #spec = reaction mix, lib = list of region data, in list for reagents
    ...
    guess = np.sum(np.stack( [synthesize(r,shifts[i],N) for i,r in enumerate(lib)], axis=1), axis=1 )
