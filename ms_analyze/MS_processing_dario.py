...
ms_lib = {0: (56.0, 42.0, 83.0),
 ...
 8: (98.0, 69.0, 166.0, 41.0, 42.0, 44.0, 140.0, 270.0, 80.0, 112.0, 82.0, 83.0, 114.0, 241.0, 56.0),
 ...
 18: (64.0, 312.0, 42.0, 141.0, 142.0, 79.0, 143.0, 145.0, 147.0, 83.0, 310.0, 56.0, 314.0),
 19: (216.0, 215.0)}
...
def get_reagent_file(n):
    ...
    '''
    msreagents_paths = [i for i in glob.glob(os.path.join(REAGENTS_FOLDER,'*_MS')) if 'BLANK' not in i]
...
class ProcessMS():
    ...
    def get_hplc_data(self):
        ...
        try:
            self.hplc_mix, self.hplc_diff, self.hplc_recon, _ = ps.filter_spectrum(name_to_HPLC(self.s))
        except NameError:
            print('no HPLC found')
            return
        self.hplc_new_p = find_peaks(self.hplc_diff / max(self.hplc_mix[:, 1]), height=0.05)# rt of NEW peaks from hplc data
        self.hplc_reactivity = False if len(self.hplc_new_p[0])==0 else True
        self.hplc_r_p = find_peaks(self.hplc_recon/max(self.hplc_recon), height = 0.05) # rt of REAGENT peaks from hplc data
    ...
    def get_nmr_data(self):
        ...
        nmr_reagents = np.array([np.real(process_nmr(name_to_NMR(get_reagent_file(reagent))).spectrum[:3921]) for reagent in self.reagents])
    ...
    def find_blanks_masses(self):
        ...
        DT = 0.5
        for p in np.take(b_TIC[0],b_TIC_p[0]): #finding ms top peaks around the TIC peaks
    ...
    def find_reagents_masses(self):
        ...
        reag_masses = []
        DT=0.5
        for p in np.take(self.hplc_mix, self.hplc_r_p[0]): #finding ms top peaks around the TIC peaks
    ...
    def filter_TIC_peaks(self):
        ...
        TIC_p_new = filter_out_peaks(np.take(self.TIC[0],self.TIC_p[0]), np.take(self.hplc_mix[:,0], self.hplc_r_p[0])) #removing peaks coming when reagents come out
        TIC_p_new = [i for i in TIC_p_new if 3.6<i<23] #removing stuff with solvent front and column fragments at the end
        TIC_p_new = remove_naphtalene_contamination(self.spec, TIC_p_new) # it persists from previous exp
        return TIC_p_new
    ...
    def plot_MS(self, ax):
        ...
        for p in self.TIC_p_filt:
            peaks_mass, peaks, found, masses = extract_peaks(self.spec, p-DT, p+DT, height = 0.3)
            ax.plot(masses, peaks, label=str(p))
            for f in found[0]:
                 ax.annotate(str(int(masses[f])), xy=(masses[f],peaks[f]), xytext=(masses[f], peaks[f]+0.01))   
    ...
    def plot_HPLC_TIC(self, ax):
        ...
        ax.plot(self.hplc_mix[:,0],(self.hplc_mix[:,1]/max(self.hplc_mix[:,1])), c="orange")
        ax.plot(self.hplc_mix[:,0], (self.hplc_diff/max(self.hplc_mix[:,1])), c="red")
        
        
        ax.scatter(np.take(self.hplc_mix[:,0], self.hplc_r_p[0]),self.hplc_r_p[1]['peak_heights'], zorder=2, c='blue')
        ax.scatter(np.take(self.hplc_mix[:,0], self.hplc_new_p[0]),self.hplc_new_p[1]['peak_heights'], zorder=2, c='red')
        ax.scatter(self.TIC_p_filt,[-0.2 for i in range(len(self.TIC_p_filt))], zorder=2, c='green')
        ...
        for p in peaks_masses:
            for g in reagents_w_couple:
                ...
                if sum(g)+23-DM<p<sum(g)+23+DM:
                    print('FOUND A MATCH WITH Na: {} - sum of {} + 23 = {}'.format(p, g, sum(g)+23))
                if sum(g)*2-DM<p<sum(g)*2+DM:
                    print('FOUND A MATCH DOUBLE MASS: {} - sum of {}*2 = {}'.format(p, g, sum(g)*2))
