...
from advion_wrapper import AdvionData

class MassSpectra:
    """
    Represents a series of mass spectra, typically from an LC/GC-MS experiment.
    """
    def __init__(self, retention_times: np.array, masses: np.array, spectra: np.array, scale=None):
    ...
    @classmethod
    def from_advion(cls, data:AdvionData):
    ...
    def discretize(self, delta_mass: float):
        ...
        for i, m in enumerate(new_masses):
            new_spectra[:, i] = self.spectra[:, (self.masses > m) & (self.masses < m + delta_mass)].sum(axis=1)
    ...
    def get_slice(self, t_start: float, t_end: float):
        ...
        return MassSpectra(self.retention_times[selector], self.masses, self.spectra[selector, :], self.scale)
    ...
    def chromatogram(self, normalize: bool=False):
        """
        Returns a 2 x N matrix representing the retention time (in minutes) and TIC
        for the given AdvionData object.
        ...
        return np.stack([self.retention_times, TICs / TICs.max() if normalize else TICs])
    ...
    def integrate(self, t_start: float, t_end: float, normalize=False):
        ...
        return MassSpectrum(self.masses, intensities/intensities.max() if normalize else intensities)
    ...
    def TIC_peaks(self, height=0.2, peak_Δt=0.5, prominence=0.15, **options):
        ...
        return find_peaks(chromatogram, height=height, distance=int(peak_Δt/self.time_resolution), prominence=prominence, **options)[0]
    ...
    def find_components(self, num_components: int):
        ...
        return model.components_.T, [MassSpectrum(self.masses, transformed[:, i]) for i in range(num_components)], error
